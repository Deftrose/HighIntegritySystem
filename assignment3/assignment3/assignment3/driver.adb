...
procedure Driver with SPARK_Mode is
   ...
   test3 : Program := (1 => (OP => MOV, MovRd => 1, MovOffs => 0), 2 => ( OP => MOV, MovRd => 2, MovOffs => 1),
                       3 => (OP => ADD, AddRs1 => 5, AddRs2 => 4, AddRd => 3), 4 => (OP => RET , RetRs => 3), others => (OP => NOP));
begin
   ...
   HasInvalidBehaviour := DetectInvalidBehaviour(test3,MAX_PROGRAM_LENGTH);
   Put("Analysis Result: ");
   ...
   ExecuteProgram(test3,MAX_PROGRAM_LENGTH,Code,Result);
   Put("Return Code: ");
   Put(Code'Image);
